...
def splitnominee(n):
    if n in[1, 2, 9, 12, 13, 14]:
        return splitaward(n) + ["picture", "nomin"]
    return splitaward(n) + ["nomin"]
...
def parsenomiees_name(n):
    dic ={}
    awardlist = splitnominee(n)
    while 1:
        line = file.readline()
        count = 0
        total = len(awardlist)
...
def filter_winner(dict):
    ...
    while i < len(dict):
        ...
        if j >= len(forbiddenlist):
            return i
        i+=1
index = filter_winner(res)
print "winner goes to" + res[index]
#get nominees name
def filter_nominees(res):
    forbiddenlist = ["Best","Actor","Actress","Drama","Golden","Globe","Motion","Film","Director","La","Wiki"]
    nominees = res
    for fword in forbiddenlist:
        nominees = list(filter(lambda w: fword not in w, nominees))
    if len(nominees) < 6:
        length = len(nominees)
    else:
        length = 6
    return nominees[:length]
print 'nominees', filter_nominees(res)
